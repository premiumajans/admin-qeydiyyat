                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center"><img src="{{ asset($team->image) }}" width="100"
                                    height="50"></td>
                                <td class="text-center">{{ $team->translate('az')->name }}</td>
                                <td class="text-center">{{ $team->translate('az')->position }}</td>
                                <td class="text-center">{{ $team->alt }}</td>
                                <td>{{ date('d.m.Y H:i:s',strtotime($team->created_at))}}</td>
                                <td class="text-center">
                                    <a class="btn btn-primary"
                                        href={{ route('backend.team.edit',$team->id) }}>
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a class="btn btn-danger"
                                        href="{{ route('backend.team.destroy',$team->id) }}">
